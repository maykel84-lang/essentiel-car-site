export async function onRequestPost(context) {
  ...
  if (!key) {
    return json({ error: 'Paiement non configuré. Contactez le support.' }, 500);
  }
  ...
  const validItems = items.filter(item => item.name && item.price > 0 && item.qty > 0);
  if (validItems.length === 0) {
    return json({ error: 'Articles invalides' }, 400);
  }
  ...
  validItems.forEach((item, i) => {
    ...
    if (item.image) {
      body.append(`line_items[${i}][price_data][product_data][images][]`, item.image);
    }
  });
  ...
  try {
    ...
    if (!stripeRes.ok) {
      const msg = session.error?.type === 'StripeAuthenticationError'
        ? 'Clé API Stripe invalide. Vérifiez la configuration.'
        : session.error?.type === 'StripeInvalidRequestError'
          ? `Erreur Stripe : ${session.error.message}`
